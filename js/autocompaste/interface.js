...
AutoComPaste.Interface = (function () {
   ...
   */
  function Interface (wm, engine, texts_json, stimuli, numOfWindows) {
    /** Internal functions */
      ...
      if (privates.texts_returned == privates.texts_available) {
        ...
        for (var text_title in privates.texts) {
          if (privates.texts.hasOwnProperty(text_title)) {
			...
			//Creates a single window for texts that contain the stimuli, also creates double windows for testing
			var singleWindowMode = privates.numOfWindows == "Single Window";
			var doubleWindowMode = privates.numOfWindows == "Double Windows";
			var quintupleWindowMode = privates.numOfWindows == "Quintuple Windows";
			var hasStimuli = privates.texts[text_title].includes(privates.stimuli);
			if(singleWindowMode) {
				if(hasStimuli) {
                    iface._createWindowForText(text_title);
				}
			} else if(doubleWindowMode) {
                if(hasStimuli) {
                    iface._createWindowForText(text_title);
                } else {
                    if(createdWindow) {
    ...
    privates.numOfWindows = numOfWindows;
